    ...
    public class UgovorRepo
    {
        ...
        public UgovorModel? DajUgovorPoId(int id)
        {
            ...
            if (citac.Read())
            {
                ...
                ugovor.StavkeUgovora = new List<StavkaUgovoraModel>();
            }
            ...
            if (ugovor != null && citac.NextResult())
            {
                while (citac.Read())
                {
                    ugovor.StavkeUgovora.Add(new StavkaUgovoraModel
                    {
                        Id = citac.GetInt32(
                            citac.GetOrdinal("Id")),
        ...
        public int DajBrojZavrsenihUgovoraKlijenta(int klijentId)
        {
            const string upit = @"
                SELECT COUNT(*)
                FROM Ugovori
        ...
        private static UgovorModel MapirajUgovor(SqlDataReader citac)
        {
            return new UgovorModel
            {
                ...
                BrojUgovora =
                    citac.GetString(citac.GetOrdinal("BrojUgovora")),

                DatumIzdavanja =
                    citac.GetDateTime(citac.GetOrdinal("DatumIzdavanja")),

                DatumPreuzimanja =
                    citac.GetDateTime(citac.GetOrdinal("DatumPreuzimanja")),

                DatumVracanja =
                    citac.GetDateTime(citac.GetOrdinal("DatumVracanja")),

                MestoPreuzimanja =
                    citac.GetString(citac.GetOrdinal("MestoPreuzimanja")),

                MestoVracanja =
                    citac.GetString(citac.GetOrdinal("MestoVracanja")),

                NacinPlacanja =
                    citac.GetString(citac.GetOrdinal("NacinPlacanja")),

                Depozit =
                    citac.GetDecimal(citac.GetOrdinal("Depozit")),

                StatusUgovora =
                    citac.GetString(citac.GetOrdinal("StatusUgovora")),

                Napomena =
                    citac.IsDBNull(citac.GetOrdinal("Napomena"))
                        ? null
                        : citac.GetString(citac.GetOrdinal("Napomena")),

                PopustProcenat =
                    citac.GetDecimal(citac.GetOrdinal("PopustProcenat")),

                UkupnoZaPlacanje =
                    citac.GetDecimal(citac.GetOrdinal("UkupnoZaPlacanje")),

                KlijentId =
                    citac.GetInt32(citac.GetOrdinal("KlijentId")),

                KorisnikId =
                    citac.GetInt32(citac.GetOrdinal("KorisnikId"))
            };
        }
    }
}