...
namespace BibliotekaKlasa.KlasePodataka.Repozitorijumi
{
    public class VoziloSPRepo
    {
        ...
        {
            using SqlConnection konekcija = new SqlConnection(_stringKonekcije);
            using SqlCommand komanda = new SqlCommand("spIzmeniVozilo", konekcija);

            komanda.CommandType = CommandType.StoredProcedure;
            komanda.Parameters.Add("@Id", SqlDbType.Int).Value = vozilo.Id;

            DodajParametreVozila(komanda, vozilo);

            konekcija.Open();

            int brojPromenjenihRedova = komanda.ExecuteNonQuery();
        ...
        {
            using SqlConnection konekcija = new SqlConnection(_stringKonekcije);
            using SqlCommand komanda = new SqlCommand("spObrisiVozilo", konekcija);
